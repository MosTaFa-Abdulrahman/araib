  ...
  return (
    <div className={`getReturnById ${theme}`}>
      <div className="content-grid">
        ...
        <div className="details-card">
          <div className="titleContent">
            <h2 className="card-title">{t("Return Invoice Details")}</h2>
            <div className="statusContainer">
              <span
                className={`status-chip ${singleReturnInvoice?.paymentStatus}`}
              >
                {t(singleReturnInvoice?.paymentStatus)}
              </span>
            </div>
          </div>
          ...
          <div className="details-content">
            <div className="detail-row">
              <span className="label">{t("Return Invoice Number")}</span>
              <span className="value">{singleReturnInvoice.invoiceNumber}</span>
            </div>
            <div className="detail-row">
              <span className="label">{t("Invoice PO Number")}</span>
              <span className="value">PO-17</span>
            </div>

            <div className="detail-row">
              <span className="label">{t("Supplier Name")}</span>
              <span className="value">
                {singleReturnInvoice?.supplier?.name}
              </span>
            </div>
            <div className="detail-row">
              <span className="label">{t("Location")}</span>
              <span className="value">
                {singleReturnInvoice?.stockLocationName}
              </span>
            </div>
            <div className="detail-row">
              <span className="label">{t("Issue Date")}</span>
              <span className="value">
                {new Date(singleReturnInvoice?.issueDate).toLocaleString()}
              </span>
            </div>
            <div className="detail-row">
              <span className="label">{t("Notes")}</span>
              <span className="value">{singleReturnInvoice?.notes || "-"}</span>
            </div>
          </div>
          ...
          <div className="attachments-section">
            ...
            <div className="attached-files">
              {[...singleReturnInvoice.attachments, ...attachments].map(
                (file) => (
        ...
        <div className="products-card">
          ...
          <div className="products-table">
            <table>
              ...
              <tbody>
                {singleReturnInvoice?.stockOrderItems?.map((product) => (
            ...
            {showTotals && (
              <div className="totals-content">
                <div className="total-row">
                  ...
                  <span>
                    {formatCurrency(singleReturnInvoice?.subTotalTaxExclusive)}
                  </span>
                </div>
                <div className="total-row discount">
                  <span>{t("Discount Amount")}</span>
                  <span>
                    - {formatCurrency(singleReturnInvoice?.discountAmount)}
                  </span>
                </div>
                <div className="total-row tax">
                  <span>{t("Total Tax")}</span>
                  <span>+ {formatCurrency(singleReturnInvoice?.totalTax)}</span>
                </div>
                <div className="total-row final">
                  ...
                  <span>
                    {formatCurrency(singleReturnInvoice?.totalTaxInclusive)}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="paidContainer">
            ...
            <div className="paidContent">
              ...
              <div style={{ color: "tomato" }}>
                {formatCurrency(
                  singleReturnInvoice?.totalTaxInclusive -
                    singleReturnInvoice?.paidAmount || ""
                )}
              </div>
            </div>
            <div className="paidContent">
              ...
              <div>
                {new Date(singleReturnInvoice?.paymentDueDate).toLocaleString()}
              </div>
            </div>
            <div className="paidContent">
              ...
              <div>
                {new Date(
                  singleReturnInvoice?.expectedDeliveryDate
                ).toLocaleString()}
              </div>
            </div>
          </div>
      ...
      <div className="action-buttons">
        ...
        {singleReturnInvoice?.paymentStatus === "fully-paid" ? (
          ""
        ) : (
