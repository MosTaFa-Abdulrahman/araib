...
function SpecialProductModal({
  ...
  // Update useEffect for initial items loading
  useEffect(() => {
  ...
  // Handle Get Batch and Serial Numbers
  const getBatchOrSerialNumbers = () => {
    if (!product?.ProductVariantToStockLocations?.[0]?.VariantToTracks) {
      return [];
    }

    const sourceVariantToTracks =
      product.ProductVariantToStockLocations[0].VariantToTracks;

    // Filter out tracks with quantity > 0
    const availableTracks = sourceVariantToTracks.filter((track) => {
      // For serial numbers, check if it's not already selected
      if (product.trackType === "serial") {
        const isUsed = items.some(
          (item) => item.serialNumber === track.trackNo
        );
        return track.quantity > 0 && !isUsed;
      }
      // For batch numbers, just check quantity
      return track.quantity > 0;
    });
        ...
        setErrors((prev) => ({
          ...
          quantity:
            numValue <= 0
              ...
              : t(`Maximum available quantity is ${availableQty}`),
        }));
        return;
      }
